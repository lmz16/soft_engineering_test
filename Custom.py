...
class CustomC():
    def __init__(self):
        ...
        self.actionpath=[[],[],[],[]]
        extern.R_cu=Resource.RCustom('Resource/json/custom')
    ...
    def customshow2(self):
        ...
        [x,y]=pygame.mouse.get_pos()
        extern.interface_resource.screen.blit(extern.interface_resource.custom_frame_pic,(105,34))
        ...
        for tx,pic in enumerate(extern.R_cu.pic):
            rc=[int(tx/5),tx%5]
                ...
                if (event.button==1):
                    ...
                    if (ty%250)<160:
                        if ((tx%140)>20)&((tx%140)<120):
                            if (int(tx/140)+int(ty/250)*5)<len(extern.R_cu.pic):
                                self.actionpic.append(
                                    pygame.transform.smoothscale(
                                        pygame.image.load('Resource/custom/'+extern.R_cu.picpath[int(tx/140)+int(ty/250)*5]
                                        ).convert_alpha(),
                                    self.charactersize))
                                self.actionpath[self.actionkind].append('Resource/custom/'+extern.R_cu.picpath[int(tx/140)+int(ty/250)*5])
